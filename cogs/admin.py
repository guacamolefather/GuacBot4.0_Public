...
class Admin(commands.Cog):
    ...
    @bridge.bridge_command(description="Tests if the Admin cog is loaded.")
    @commands.check(is_it_me)
    async def admintest(self, ctx):
        await ctx.respond('Admin extension cog works!')

    @bridge.bridge_command(aliases=['purge', 'prune', 'delete'],description="Gets rid of a specified amount of messages.")
    ...
    @bridge.bridge_command(description="Kicks the specified member (with opt reason).")
    @commands.check(admin)
    async def kick(self, ctx, member: discord.Member, *, reason=None):
        await member.kick(reason=reason)
        await ctx.respond(f'Kicked {member.mention}.\nReason: {reason}.')

    @bridge.bridge_command(description="Bans the specified member (with opt reason).")
    @commands.check(admin)
    async def ban(self, ctx, member: discord.Member, *, reason="A good one, trust me."):
        await member.ban(reason=reason)
        await ctx.respond(f'Banned {member.mention}.\nReason: {reason}.')

    @bridge.bridge_command(description="Softbans the specified member (with opt reason).")
    @commands.check(admin)
    ...
    @bridge.bridge_command(description="Unbans the specified member (needs discriminator).")
    @commands.check(admin)
    async def unban(self, ctx, *, member):
        ...
        try:
            ...
            await ctx.respond("Please include the discriminator of the member. (username#0000)")
        ...
        await ctx.respond(f"Member {member} not found.")

    @bridge.bridge_command(description="Gives the specified role to the specified member.")
    @commands.check(admin)
    ...
    @bridge.bridge_command(description="Takes the specified role from the specified member.")
    @commands.check(admin)
    ...
    @bridge.bridge_command(description="Turns the attached or replied image into an emoji (with opt name).")
    @commands.check(admin)
    ...
    @bridge.bridge_command(description="Steals the given custom emoji for the server.")
    @commands.check(admin)
    ...
    @bridge.bridge_command(description="Shows in-depth information on a user.")
    @commands.check(admin)
    ...
    @bridge.bridge_command(description="Counts the amount of people with certain roles.")
    @commands.check(admin)
